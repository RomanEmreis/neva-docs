...
| Файл | О чём |
|---|---|
| `SKILL.md` | Определение версии и профиля, формы обработчиков, список ловушек, минимальные сервер и клиент, маршрутизация |
| `references/server.md` | Формы обработчиков (`async fn`, обычная `fn`, `blocking`), инструменты, промпты, ресурсы, схемы, имена аргументов, типы контента, возможности клиента в каждом запросе, DI, middleware, логи, подписки, рассылка между экземплярами |
