...
[CHANGELOG](https://github.com/RomanEmreis/neva/blob/main/CHANGELOG.md).
...
:::info Идентификаторы без потока тоже возобновляются
...
Изменились два вызова. Оба ломают сборку, а не меняют поведение молча, так что
глазами ничего проверять не нужно.

### `UiResource::with_permissions` переименован

На [`UiResource`](./mcp-server/apps#как-отдать-документ) `with_permissions`
...
[`async fn`, и обычную `fn`](./mcp-server/handlers). Маркер всегда выводится,
...
## Обновление 0.4.x → 0.5.0 {#migrating-to-050}
