...
MCP 2026-07-28
[заменил рукопожатие на discovery](../spec-2026-07-28#discovery-вместо-рукопожатия),
...
сам. Под [`legacy-spec`](../legacy-spec) то же объявление едет на рукопожатии
`initialize` внутри `capabilities.extensions`.

Сервер читает её через
[`Context::supports_apps()`](../mcp-server/apps#как-спросить-умеет-ли-вызывающая-сторона-рендерить)
и может менять свой `content` в зависимости от того, умеете ли вы рендерить.

:::info Появилось в 0.6.0
До 0.6.0 серверу на 2026-07-28 клиент на neva не объявлял **ничего**:
объявление жило только на легаси-пути через `initialize`, так что `with_apps()`
до такого сервера не доходил вовсе, а у обработчика не было способа спросить.
Ничему на этой странице это не мешало — чтение метаданных из `tools/list` и
`resources/read` не требует согласования, — но сервер не мог *менять* свой
ответ. Теперь может.
:::
