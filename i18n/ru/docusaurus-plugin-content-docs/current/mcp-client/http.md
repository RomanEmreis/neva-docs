...
Отдельного SSE-потока `GET` в транспорте больше нет — neva его не открывает
под капотом `connect()`. Серверные уведомления вместо этого приходят на
запросе [`subscriptions/listen`](./subscriptions): ответ на этот `POST` и есть
поток, а открывает его `Client::listen`.
...
Версия, которую клиент предлагает в этом рукопожатии, — **предложение, а не
требование**: сервер, который её не поддерживает, отвечает той, которую
поддерживает, и соединение продолжается, если клиент говорит и на ней.
Соединение обрывает только версия, о которой neva ничего не знает: настаивать
на предложенной версии значило бы отказывать каждому серверу, который на шаг
старше клиента, — а ровно ради этого случая согласование и существует.
