...
:::info `Abrupt` — не обязательно ошибка на этой стороне
Сервер, завершающий подписку по собственной инициативе, СЛЕДУЕТ сначала
отправить пустой результат, и сервер на neva так и делает — см.
[Сервер → Корректная остановка](../mcp-server/shutdown). Доставляют его не все
пиры, поэтому `Abrupt` при остановке читайте как «пир остановился», а не как
локальную ошибку.
:::
