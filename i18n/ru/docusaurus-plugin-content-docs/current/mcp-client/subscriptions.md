...
:::info При остановке сервера полагается `Graceful` — начиная с neva 0.5.4
Сервер, завершающий подписку по собственной инициативе, СЛЕДУЕТ сначала отправить
пустой результат, и до **0.5.4** neva его формировала, но почти никогда не
доставляла: один токен отмены управлял и подпиской, и транспортом, поэтому
результат гонялся с писателем, который уже вышел из своего цикла по тому же
сигналу. **0.5.5** довела это до конца: до неё `run` мог вернуть управление
внутрь дренирования, которое ещё писало, и под `run_blocking` уроненная следом
среда выполнения обрывала писателя. От более старого сервера `Abrupt` при
остановке читайте как «пир остановился», а не как ошибку на этой стороне. См.
[Сервер → Корректная остановка](../mcp-server/shutdown).
:::
