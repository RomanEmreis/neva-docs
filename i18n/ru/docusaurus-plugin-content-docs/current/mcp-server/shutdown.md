...
который запустил их дренирование, — появился в **0.5.5**. Отмена токена
транспорта раньше делала два дела сразу: писатели начинали дренировать
очередь, и собственный цикл `run` прерывался по тому же сигналу и возвращал
управление. Ничто не связывало первое со вторым, поэтому на 0.5.4 фаза подписок
работает, но под `run_blocking` недописавший писатель прерывается на середине
дренирования — то самое резкое закрытие, ради предотвращения которого
дренирование и существует. Если вы полагаетесь на корректное закрытие, возьмите
патч-релиз.
:::

## HTTP-движок тоже обязан остановиться

[`HttpEngine`](./custom-http) получает `CancellationToken` и обязан опустить
...
:::warning Исправлено в 0.5.5
Встроенный движок на Volga брал токен транспорта и использовал его только для
отчёта о собственных сбоях, поэтому слушающий сокет опускался по сигналам,
которые Volga обрабатывала сама, и ничем больше. Сервер, остановленный через
`ShutdownHandle`, а не по Ctrl+C, возвращался из `run` **с всё ещё занятым
портом, продолжая обслуживать запросы** — пока владелец среды выполнения не
уронит её, а под `tokio::spawn` это может не случиться никогда. Ctrl+C
работал. Если вы останавливаете серверы из кода, это тот фикс, который нужно
взять.
:::
