...
:::info Появилось в neva 0.6.0
До **0.6.0** обработчик обязан был возвращать future, поэтому даже тело,
состоящее из одной арифметической операции, приходилось писать как
`async fn`. Синхронные обработчики **дополняют**, а не заменяют: форма
определяется по сигнатуре, публикуемая схема и слоты аргументов в обоих
случаях одинаковы, а существующие `async fn`-обработчики трогать не нужно.
:::
...
:::warning Явно выписанным дженерикам нужен ещё один аргумент
Методы регистрации получили маркер как параметр-дженерик, поэтому вызову,
который выписывает дженерики вручную, нужен четвёртый аргумент:

```rust
// 0.5.x
app.map_tool::<_, _, (String,)>("greet", greet);

// 0.6.0 — E0107, пока маркер не добавлен
app.map_tool::<_, _, (String,), _>("greet", greet);
```

Это единственное место, ломающее исходники. Любой вызов, который оставляет
работу выводу типов — а так их и пишут, — не затронут. См.
