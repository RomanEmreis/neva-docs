...
| Тип | Предоставляет |
|-----|---------------|
| `Context` | Текущий контекст запроса (изменение реестров инструментов, промптов и ресурсов, уведомление [подписчиков](./subscriptions), запрос ввода у клиента и т.д.) |
...
Neva-серверы обрабатывают [пакетные запросы JSON-RPC 2.0](https://www.jsonrpc.org/specification#batch) автоматически — на транспортах `stdio` и HTTP — без какой-либо дополнительной настройки.
Когда клиент отправляет пакет (JSON-массив объектов запросов), сервер обрабатывает каждый запрос и возвращает ответы в виде JSON-массива в том же порядке.

Клиентский API описан в [руководстве по пакетным запросам](/docs/mcp-client/batch).
