...
Потоковый HTTP-транспорт в Neva — **подключаемый**. По умолчанию сервер построен на [Volga](https://docs.rs/volga) и включается через `server-full` / `http-server-volga`, но MCP-эндпоинт можно разместить на любом HTTP-стеке — `axum`, `hyper`, `actix-web` или собственном адаптере, — реализовав один трейт.
...
:::warning Записывайте `EventId` целиком
`tracked_event` получает
[`EventId`](https://docs.rs/neva/latest/neva/transport/http/core/types/struct.EventId.html),
а не голый порядковый номер: сессия может держать [несколько SSE-потоков
одновременно](../legacy-spec#concurrent-sse-streams), а идентификатор события —
это курсор *внутри одного потока*, поэтому он обязан называть и поток, и
позицию в нём.
...
`dispatch_post` возвращает
`Result<StreamResponse<impl Stream<Item = E::SseEvent>>, Error>`, поэтому движок
обрабатывает на POST-маршруте тот же match из двух вариантов, что уже есть у его
GET-маршрута. `handlers::handle_post` остаётся доступен как строительный блок
только для JSON. Сборки без `tracing` — как и сборки с `legacy-spec` — всегда
дают `Complete`.
