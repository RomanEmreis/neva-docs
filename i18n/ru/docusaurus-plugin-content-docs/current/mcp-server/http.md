...
SSE-потоки `GET` с воспроизведением по `Last-Event-ID` — сессия держит
столько потоков одновременно, сколько откроет клиент, у каждого свой курсор.
См. [Легаси-спецификация → Одновременные
SSE-потоки](../legacy-spec#concurrent-sse-streams).
:::
...
App::new()
    ...
    // изменении, случившемся на другом.
...
Считается ли адрес loopback, решается так же, как `std` читает строку привязки,
поэтому IPv6 без скобок тоже работает: `bind("::1:3000")` слушает `[::1]:3000`
и получает политику loopback. Скобки всё же лучше ставить — `[::1]:3000` говорит
сам за себя, не полагаясь на правило последнего двоеточия.

`Origin` с userinfo сопоставляется по настоящему хосту, а не по имени перед
`@`: у `https://app.example.com:8443@evil.com` хост — `evil.com`.
