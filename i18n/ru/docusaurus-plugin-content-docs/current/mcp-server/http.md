...
* Нет отдельного SSE-потока `GET` для серверных уведомлений.
...
`POST` по-прежнему получает ответ `text/event-stream`, если он подписался на
поток — то есть если в его `_meta` есть `io.modelcontextprotocol/logLevel`
или `progressToken`. Поток несёт уведомления
[журнала](./logging) и [прогресса](./progress) именно этого запроса, а следом
— его ответ. Все остальные `POST` получают один JSON-объект.
...
два общих ресурса становятся обязательными:
