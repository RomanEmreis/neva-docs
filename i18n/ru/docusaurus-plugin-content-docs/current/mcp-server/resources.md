...
Обработчик чтения ресурса или списка ресурсов тоже может быть обычной `fn`.
Чтение с диска *блокируется*, так что это хрестоматийный случай для
`#[resource(blocking)]` или `neva::blocking`: тело уедет на blocking-пул
Tokio вместо того, чтобы удерживать воркер рантайма. См.
[Формы обработчиков](./handlers).
:::
