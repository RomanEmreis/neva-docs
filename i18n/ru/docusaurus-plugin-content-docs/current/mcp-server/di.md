...
Neva включает встроенный контейнер внедрения зависимостей (DI), позволяющий регистрировать общие сервисы — подключения к базам данных, HTTP-клиенты, объекты конфигурации, кэши — и автоматически предоставлять их обработчикам инструментов, ресурсов и запросов.
...
Используйте `Dc<T>` как параметр функции для получения сервиса в любом обработчике инструмента, ресурса или запроса. `Dc` (Dependency Container) оборачивает разрешённый экземпляр в `Arc` и реализует `Deref`, поэтому его можно использовать как обычную ссылку.
