...
:::info Корректное закрытие при остановке
...
упорядоченное завершение от оборванного соединения.

Именно поэтому остановка [двухфазная](./shutdown#что-происходит-при-остановке).
...
выполнения. [`App::with_shutdown_drain(..)`](./shutdown) ограничивает это
