...
:::warning Два `permissions`, два смысла
`with_ui_permissions` запрашивает у браузера разрешения **для iframe** — камеру,
геолокацию и прочее. `with_permissions` говорит, **кто может прочитать ресурс**,
и проверяется по claims вызывающей стороны, как на любом другом ресурсе. Они
никак не связаны, и отказать кому-либо может только второй.
:::
...
Опечатка в атрибуте ловится так же: макросы
[отклоняют незнакомый атрибут](./tools#unknown-attributes-are-rejected), а не
игнорируют его, — именно это не даёт опечатке в `visibility` опубликовать агенту
инструмент, предназначенный только для приложения.
...
Регистрация документа через `map_ui_resource` с требованием на возвращённом
`ResourceTemplate` делает то же самое — если разметка генерируется, а не
фиксирована.

Оба билдера доступны под фичей `http-server`: роли и права приходят из
проверенного токена, а это забота HTTP-транспорта.
...
Клиент объявляет это вызовом `with_apps()`, после чего карта едет в
[`_meta` каждого запроса](../spec-2026-07-28#capabilities-ride-each-request) —
это общий механизм, которым MCP Apps просто пользуется.

:::note Недоступно под `legacy-spec`
Там в `_meta` нет канала для возможностей — объявление едет в `initialize`, а
`supports_apps` из сборки вырезается.
:::
