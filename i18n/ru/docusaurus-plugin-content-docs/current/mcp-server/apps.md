...
| `with_ui_permissions` | Разрешения браузера, которые iframe *запрашивает* |
...
| `with_roles` / `with_permissions` | Кто может **прочитать** ресурс — см. [Авторизацию](#авторизация) |

:::warning В 0.6.0 `with_permissions` сменил смысл
На `UiResource` он теперь задаёт, **кто может прочитать ресурс**, как и на
любом другом ресурсе. Разрешения браузера для iframe переехали в
**`with_ui_permissions`**, поэтому вызов `with_permissions(UiPermissions::..)`
из 0.5.6 больше не компилируется. Переименование механическое — см.
[переход на 0.6.0](../spec-2026-07-28#migrating-to-060).
`UiResourceMeta::with_permissions` не менялся.
:::
...
Ресурс, зарегистрированный через `add_ui_resource`, **по умолчанию не несёт
требований к ролям или правам**: на [защищённом OAuth сервере](./oauth) его
прочтёт любой, кто до него дотянется.

Обычно это правильно. Документ — шаблон, который хост как раз должен заранее
забрать и проверить при подключении, а данные для него приходят из инструмента,
у которого есть собственное требование.

Если чувствительна сама разметка, задайте требование прямо на ресурсе через
`with_roles` и `with_permissions` — ту же пару, что
[`ResourceTemplate`](./oauth#роли-и-права) и
`#[resource(roles = [..])]` вешают на любой другой ресурс, и проверяется она в
том же месте:
...
Вызывающей стороне, у которой нет ни одной из ролей, `resources/read` ответит
отказом. Вместе `with_roles` и `with_permissions` должны быть удовлетворены
**оба**.

:::info Появилось в 0.6.0
До 0.6.0 ограничить `ui://`-документ можно было только регистрацией через
`map_ui_resource` с требованием на возвращённом `ResourceTemplate`. Так тоже
по-прежнему можно; теперь `add_ui_resource` несёт ту же пару напрямую.

Оба билдера доступны под фичей `http-server` — роли и права приходят из
проверенного токена, а это забота HTTP-транспорта. Не путайте
`with_permissions` (кто может читать) с
[`with_ui_permissions`](#как-отдать-документ) (что iframe может попросить у
браузера).
:::

`resources/read` проверяет требование на **сматченном маршруте**, а не ищет и
копирует шаблон ресурса на каждое чтение: требование шаблона переносится на его
маршруты при старте сервера.
...
## Как спросить, умеет ли вызывающая сторона рендерить

Обработчик может узнать, есть ли у *этой* вызывающей стороны iframe, и
подобрать `content` под аудиторию:

```rust compile features="server-full"
use neva::prelude::*;

#[tool(descr = "Текущее время", ui = "ui://clock/app.html")]
async fn get_time(ctx: Context) -> String {
    let now = "12:00:00 UTC";
    if ctx.supports_apps() {
        // Показывать будет приложение — отдаём голое значение.
        now.to_string()
    } else {
        // Кроме текста ничего не будет — отвечаем предложением.
        format!("Сейчас {now}.")
    }
}
...
истинен только если вызывающая сторона объявила расширение **и** её
`mimeTypes` называет `text/html;profile=mcp-app`. `mimeTypes` спецификация
требует обязательно, так что объявление без него не считается.

:::warning Это меняет форму ответа, но не отменяет сам ответ
Инструмент с UI обязан вернуть осмысленный `content` в любом случае: `content`
читает модель, а iframe есть не у каждой вызывающей стороны. Здесь вы выбираете
между голым значением и развёрнутой фразой, а не между ответом и его
отсутствием.
:::

:::info Появилось в 0.6.0
Обе половины приехали вместе: клиент на neva теперь пишет свою карту
`extensions` в `_meta` каждого запроса, так что `with_apps()` доходит до
сервера на 2026-07-28 без всякого рукопожатия, а сервер читает её через
`Context`. Раньше объявление уезжало только в `initialize` под `legacy-spec`, а
у обработчика не было способа спросить. Общий механизм — в разделе
[возможности едут в каждом запросе](../spec-2026-07-28#capabilities-ride-each-request);
MCP Apps — одно из расширений, которые им пользуются.

Недоступно под [`legacy-spec`](../legacy-spec): там в `_meta` нет канала для
возможностей, и объявление едет в `initialize`, как и раньше.
:::
