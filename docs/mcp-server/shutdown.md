...
For those, [`App::with_shutdown()`](https://docs.rs/neva/latest/neva/app/struct.App.html#method.with_shutdown),
`with_shutdown_signal(..)` and `with_shutdown_drain(..)` give you the stop
explicitly — aborting the server task instead would skip every graceful path by
construction.
