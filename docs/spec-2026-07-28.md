...
Progress, task status and elicitation are **not** subscribable; they stay
...
`sampling`, and `roots` flags; the server gates each kind on its own flag
and reports a request for an undeclared kind instead of stalling the
round-trip. The flags are additive, so a peer that only sends `elicitation`
still decodes.
...
* **[Release notes (v0.5.1)](https://github.com/RomanEmreis/neva/releases/tag/0.5.1)** and the [CHANGELOG](https://github.com/RomanEmreis/neva/blob/main/CHANGELOG.md) — the full migration narrative.
